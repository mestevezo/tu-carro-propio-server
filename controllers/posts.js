...
export const getRecommendationPosts = async (req, res) => {
  try {
    ...
    if (initialReq >= LIMIT) {
      ...
      const latestPosts = await PostMessage.find({ _id: { $nin: reqIdList } })
        .select("-othersImg -addInfo")
        .sort("-createdAt")
...
export const deletePost = async (req, res) => {
  ...
  imagekit
    .deleteFolder(process.env.IMAGEKIT_MAIN_FOLDER + folder)
    .then((response) => {
